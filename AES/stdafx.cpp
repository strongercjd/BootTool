...
/*
* Number of columns (32-bit words) comprising the State. For thisstandard, Nb = 4.
* 秘钥和明文矩阵一行有多少字节，固定为4(32-bit words)
*/
int Nb = 4;

/*
* Number of 32-bit words comprising the Cipher Key. For thisstandard, Nk = 4, 6, or 8.
*主要是秘钥矩阵有多少行
*/
...
/*
* Number of rounds, which is a function of  Nk  and  Nb (which isfixed). For this standard, Nr = 10, 12, or 14.
*Nr为给定密钥大小的轮数减 1。要加密多少轮，加密算法使用的轮数要么是10，12，要么是14，这依赖于种子密钥长度是128位、192位还是256位
*/ 
...
/**************************封装的加密函数*****start*********************/
int aes_encryption(unsigned char * date)
{
	uint8_t date_in[16];
	uint8_t date_out[16];

	memcpy(date_in, date, 16);
	cipher(date_in, date_out, w);//解密
	memcpy(date, date_out, 16);
...
/**************************封装的解密函数*****start*********************/
int aes_decrypt(unsigned char *date)
{
	uint8_t date_in[16];
	uint8_t date_out[16];

	memcpy(date_in, date, 16);
	inv_cipher(date_in, date_out, w);//解密
	memcpy(date, date_out, 16);
	return 1;
}
